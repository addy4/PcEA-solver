...
void readData (char * inputfilename) {
	...
	for (int i=0; i<incompatibilities; i++) {
		...
		fscanf(fp,"%s",temp);
		incompatiblepairs[k].x -= 1; incompatiblepairs[k].y -= 1;
		k++;
...
/* Other allocations */
	...
	totalDemand = 0; for (int i=0; i<stores; i++) totalDemand += goods[i];
...
int notcorrectgbest() {
	...
	for (i=0; i<stores; i++) A[i] = 1;
	for (i=0; i<warehouses; i++) B[i] = 1;
	for (i=0; i<stores; i++) A[gBestSolution[i]]--;
	for (i=0; i<warehouses; i++) B[gBestSolution[i+stores]]--;
	for (i=0; i<stores; i++) x+=A[i];
	for (i=0; i<warehouses; i++) x+=B[i];
	free(A);
	free(B);
	return x;
}
...
void initpop() {
	...
	for (j=0; j<stores; j++) *(pop+j) = j;
	k=0; for (j=stores; j<(stores+warehouses); j++) { *(pop+j) = k; k++; }
...
void reinit() {
	...
	for (j=0; j<(stores+warehouses); j++) *(pop+j) = gBestSolution[j];	
...
int findWH(int s, int * whidx, int wh[]) {
	...
	if (bestidx==-1) { // nothing feasible so far
		...
		} else { // if whidx has reached the end (i.e., all warehouses are open / whidx==warehouses) 
...
void fitness (int str[], int *solcost, int *nviolations) {
	...
	for (i=0; i<warehouses; i++) openwarehouses[i]=0;

	// Make a copy of the capacities array, called fcapacities[]
	for (i=0; i<warehouses; i++) fcapacities[i] = capacities[i];

	// Make a copy of the goods array, called demand[]
	for (i=0; i<stores; i++) demand[i] = goods[i];
	...
	solcount=0;
	for (i=0; i<(stores+warehouses); i++){ // put the store number in x, wh in y
		sol1[i].x = sol1[i].y = -1;
		sol1[i].v=0;
	}
	...
	for (i=0; i<stores; i++) st[i] = str[i]; 
	j=0; for (i=stores; i<(stores+warehouses); i++) { wh[j] = str[i]; j++;}
	...
	while (idx < stores) {
			...
			while (demand[i]!=0) {
				...
				if (openwarehouses[j]==0) openwarehouses[j] = 1;
				...
				if (demand[i] <= fcapacities[j]){ 
					...
					*(sol+i*warehouses+j) += demand[i];
					...
					{sol1[solcount].x = i; sol1[solcount].y = j; sol1[solcount].v+=demand[i];}
					fcapacities[j] -= demand[i];
					demand[i] = 0;
					solcount++;
				} else { // if (demand[i] > fcapacities[j])
					...
					*(sol+i*warehouses+j) += fcapacities[j];
					...
					{sol1[solcount].x = i; sol1[solcount].y = j; sol1[solcount].v+=fcapacities[j];}					
...
void finalfitness (int str[], int *solcost, int *nviolations) {
	...
	for (i=0; i<warehouses; i++) costWH[i]=0; // initalize all costs to zero
...
// Order and Alternation Crossover operators
 ...
 void crossover_order (int *p1, int *p2, int MAX) {
  ...
  if (mark_l > mark_r) { // ensure that mark_l < mark_r
     temp = mark_l;
     mark_l = mark_r;
     mark_r = temp;
  }
  ...
  for ( i=mark_l; i<mark_r; i++) { o1[i] = p1[i]; a[p1[i]]=1;}
  ...
  for ( i=mark_l; i<mark_r; i++) { o2[i] = p2[i]; a[p2[i]]=1;}
...
int _2_T () {
	...
	if ((violations[i]>0)&&(violations[j]>0)) return ((violations[i]<violations[j])?i:j);
	...
	if ((violations[i]==0)&&(violations[j]==0)) return ((popFitness[i]<popFitness[j])?i:j);
...
int ealoop () { // returns 1 if the best solution improved this generation, 0 otherwise
 ...
 if (popsel<STOREBIAS) { // stores

	if (rand()%100<XPROB) operation = XOVER;
	else 
	operation = MUTATION;

	if (operation==XOVER) { // perform crossover, select the better fit of the two individuals
		...
		for (int x=0; x<stores; x++) temp[x] = o1[x];
		for (int x=stores; x<stores+warehouses; x++) temp[x] = *(pop+p1*(stores+warehouses)+x);
		fitness(temp,&f,&v);	
		minv = v;
		minf = f;
		for (int x=0; x<stores+warehouses; x++) ofs[x]=temp[x];
		
		for (int x=0; x<stores; x++) temp[x] = o1[x];
		for (int x=stores; x<stores+warehouses; x++) temp[x] = *(pop+p2*(stores+warehouses)+x);
		fitness(temp,&f,&v);
		if ((v == minv) && (f < minf)) {
			minf = f;
			for (int x=0; x<stores+warehouses; x++) ofs[x]=temp[x];
		...
		for (int x=0; x<stores; x++) temp[x] = o2[x];
		for (int x=stores; x<stores+warehouses; x++) temp[x] = *(pop+p1*(stores+warehouses)+x);
		fitness(temp,&f,&v);
		if ((v == minv) && (f < minf)) {
			minf = f;
	...
	if (operation==XOVER) { // perform crossover, select the better fit of the two individuals
		...
		minv=INF, minf=INF;
		for (int x=0; x<stores; x++) temp[x] = *(pop+p1*(stores+warehouses)+x);
		for (int x=stores; x<stores+warehouses; x++) temp[x] = o1[x-stores];
		fitness(temp,&f,&v);
		minv = v;
		minf = f;
		for (int x=0; x<stores+warehouses; x++) ofs[x]=temp[x];

		for (int x=0; x<stores; x++) temp[x] = *(pop+p2*(stores+warehouses)+x);
		for (int x=stores; x<stores+warehouses; x++) temp[x] = o1[x-stores];
		fitness(temp,&f,&v);
		if ((v == minv) && (f < minf)) {
			minf = f;
 ...
 if ((minv==gBestViolations)&&(minf<gBestFitness)) {
	gBestFitness = minf;
	for (int x=0; x<stores+warehouses; x++) gBestSolution[x] = ofs[x];
...
if (notcorrectgbest()) {printf("\nFail inside EALoop():ofs2\n");exit(1);}	
 ...
 if (((minv==worstv)&&(minf<worstf))||(minv<worstv)) {
	//worstf = minf;
	for (int x=0; x<stores+warehouses; x++) *(pop+worstidx*(stores+warehouses)+x) = ofs[x];
